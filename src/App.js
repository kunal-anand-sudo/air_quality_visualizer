    ...
    return (
      <div className="opacity-0 hover:opacity-100 flex-col">
        <svg
          ...
          className="h-8 w-6 text-gray-400 cursor-pointer"
          fill="black"
          viewBox="0 0 24 24"
          stroke="currentColor"
          onClick={_onClick}
          onMouseOver={_onClick}
    ...
    const fetchPollutionData = async () => {
      try {
        let res = await axios.get(
          "https://api.data.gov.in/resource/3b01bcb8-0b14-4abf-b6f2-c1bfd384ba69?api-key=579b464db66ec23bdd00000168089b9538544a4c7f0c3008d2c6919a%20&format=json&offset=0&limit=1000"
        );
        ...
        for (let rec of res) {
          ...
          if (mp.has(rec.station)) {
            arr[mp.get(rec.station)].properties.avgPoll += parseInt(
              rec.pollutant_avg
            );
            ...
            arr2.push({
              ...
              geometry: {
                coordinates: coordinateJSON[rec.station],
                type: "Point",
              },
            });
            arr.push({
              ...
              properties: { avgPoll: parseInt(rec.pollutant_avg) },
            });
